"""
Command-line script for hyperparameter optimization of the TGCE_SSPS loss function.

This script executes Bayesian optimization over the `q` hyperparameter using Optuna,
evaluating multiple trials with short training runs to find the optimal truncation
...
def run_tgce_ssps_optimization(
    config_path: str,
    n_trials: int = 50,
    n_epochs_per_trial: int = 5,
    output_dir: str = "outputs/optuna/tgce_ssps"
) -> None:
    """Runs Bayesian optimisation of the TGCE_SSPS q hyperparameter."""
    
    print(f"\nStarting TGCE-SSPS Optimization (Trials: {n_trials}, Epochs per trial: {n_epochs_per_trial})")
    ...
    print(f"\nOptimization complete. Best q: {results['best_q']}")
    ...
    try:
        ...
        # Note: If there's only 1 parameter, importance might just be 1.0, but still nice to plot
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="TGCE-SSPS q Optimization Script")
    ...
    parser.add_argument("--output_dir", type=str, default="outputs/optuna/tgce_ssps", help="Output directory")
    args = parser.parse_args()
    
    run_tgce_ssps_optimization(
