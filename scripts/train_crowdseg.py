...
def run_crowdseg_experiment(config_path: str) -> None:
    """Trains and evaluates CrowdSeg on multi-annotator segmentation data.
    ...
    criterion = NoisyLabelLoss(
        num_annotators=cfg.model.num_annotators,
    ...
    tracker.print_full_report("CrowdSeg Test — Probabilistic", prob_metrics, class_names)
    ...
    if cfg.data.load_ground_truth:
        ...
        tracker.print_full_report("CrowdSeg Test — Ground Truth", gt_metrics, class_names)

    # ------------------------------------------------------------------ #
    # 7. Save weights                                                     #
    # ------------------------------------------------------------------ #
    save_file = output_path / "crowdseg_final.pth"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="CrowdSeg Training & Evaluation Script"
    )
    parser.add_argument(
        "--config",
        type=str,
        default="configs/training/crowdseg/oxfordiiitpet.yaml",
