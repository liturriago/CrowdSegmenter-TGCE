...
AnnotHarmony is a crowd segmentation model that combines a ResNet34 backbone with a ResUNet-style 
...
Key Features:
    ...
    - Attention-based annotator module for multi-annotator fusion
    - Configurable number of annotators
    - Configurable activation functions for segmentation and annotator heads
"""
...
class AttentionAnnRel(nn.Module):
    ...
    Args:
        ...
        activation: Optional activation function after segmentation head.
    """
    def __init__(self, in_channels: int, num_annotators: int, activation: Optional[str] = None) -> None:
        super().__init__()
    ...
    def forward(self, x: torch.Tensor, multihot: torch.Tensor) -> torch.Tensor:
        ...
        """ 
        # generate parameters from multihot encoding
        params = self.attention_gen(multihot)  # B × 2C
...
class AnnotHarmony(nn.Module):
    ...
    Example:    
        >>> model = AnnotHarmony(config: ModelConfig)
        >>> 
    ...
    def __init__(self,config: ModelConfig) -> None:    
        ...
        # Annotator head
        self.annotator_head = AttentionAnnRel(
            in_channels=config.decoder_channels[-1], 
            num_annotators=config.num_annotators, 
            activation=config.annotator_activation
        )
    ...
    def forward(self, x: torch.Tensor, multihot: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        Args:
            ...
            multihot: Multi-hot encoded annotator labels of shape (batch_size, num_annotators).
